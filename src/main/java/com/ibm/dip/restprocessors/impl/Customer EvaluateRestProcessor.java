...
@Service
public class Customer EvaluateRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Customer EvaluateRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.customer evaluate.hosturl}")
	private String url;
	
	@Value("${spring.api.customer evaluate.context-path}")
	private String contextPath;

	@Value("${spring.api.customer evaluate.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Customer EvaluateRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Customer EvaluateRestProcessor.class.getName());
