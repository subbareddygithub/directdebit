...
@Service
public class Direct DebitRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Direct DebitRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.direct debit.hosturl}")
	private String url;
	
	@Value("${spring.api.direct debit.context-path}")
	private String contextPath;

	@Value("${spring.api.direct debit.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Direct DebitRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Direct DebitRestProcessor.class.getName());
